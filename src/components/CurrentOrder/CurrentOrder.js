...
function CurrentOrder({makeOrder, ...props}) {
    ...
    const {orders, setOrders, orderItems, setOrderItems, addOrder} = useContext(OrderContext);
    ...
    const generatedOrderId = () => {
      ...
      const calculateTotalPrice = () => {
        ...
        const totalPrice = makeOrder.reduce((total, item) => total + (item.price * item.count), 0);
  ...
  return  ReactDOM.createPortal(
    ...
    <select id="table-select"  value={selectedTable} onChange={(e) => setSelectedTable(e.target.value)} required>
                         <option value="">Select a table</option>
                     {props.tableData.map(table => (
                         <option key={table.id} value={table.id} disabled={getOrderStatus(table.id) === 'pending'}>
                             Table {table.id}
                         </option>
                     ))}
    </select>
    ...
    {makeOrder.map((item) => (
         ...
         <button className="remove-item" onClick={() => props.removeItem(item.id)}>x</button>
       </div>
    ))}
    </div>
    ...
    <button className='proceed-order' onClick={handleAddOrder}>Proceed</button>
    <button className='order-box-close' onClick={()=>props.closeOrderBox()}>X</button>
  </div>,
